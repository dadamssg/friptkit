...
let currentScript: ChildProcess | null;
function startScript(event: Electron.IpcMainEvent) {
  console.log('killing');
  currentScript?.kill();
  currentScript = null;
  ...
  currentScript.on('message', (data) => {
    console.log('onMessage', data);
    event.reply('my-channel', data);
  });
  currentScript.on('beforeExit', () => {
    console.log('child process beforeExit');
  });
}

ipcMain.on('my-channel', async (event, arg) => {
  console.log(arg);
  if (arg[0].type === 'app:run_script') {
    startScript(event);
  }
